...
def plot_latency_bandwidth(cases, save=None):
  ...
  ax[0].grid()
  ax[0].set_xlabel('Message size (bytes)',
                   fontname='DejaVu Serif', fontsize=12)
  ...
  ax[0].set_yscale('log')
  ax[1].grid()
  ax[1].set_xlabel('Message size (bytes)',
                   fontname='DejaVu Serif', fontsize=12)
  ...
  ax[1].set_xscale('log')
  ax[1].set_yscale('log')
  ax[0].legend(loc='upper left', prop={'family': 'serif', 'size': 12})
  for a in ax:
  ...
  fig.tight_layout()
  if save:
    fig.savefig(os.path.join(os.getcwd(), save), dpi=100)
...
case2 = {'label': 'Azure Batch (NC24r)'}
keys = ['latency', 'bandwidth']
directory = os.path.join(root_dir, 'batchshipyard', 'osubenchmarks',
                         'infiniband')
series = []
for i in range(1, 6):
  tmp_case = {}
...
case3 = {'label': 'Colonial One'}
keys = ['bandwidth', 'latency']
directory = os.path.join(root_dir, 'colonialone', 'osubenchmarks')
series = []
for i in range(0, 5):
  tmp_case = {}
...
save_path = os.path.join(figures_dir,
                         'latencyBandwidthColonialOneAzure_parcfd2018.png')
plot_latency_bandwidth([case2, case3],
                       save=save_path)
