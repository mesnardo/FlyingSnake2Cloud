...
Azure with an without InfiniBand.
"""
...
case = {'label': 'Azure NC24r (dapl)'}
keys = ['latency', 'bandwidth']
directory = os.path.join(root_dir, 'infiniband')
series = []
for i in range(1, 6):
  tmp_case = {}
  for key in keys:
...
case2 = {'label': 'Azure NC24r (tcp)'}
keys = ['latency', 'bandwidth']
directory = os.path.join(root_dir, 'noinfiniband')
series = []
for i in range(1, 6):
  tmp_case = {}
  for key in keys:
...
save_path = os.path.join(figures_dir, 'latencyBandwidthInfiniBand.png')
plot_latency_bandwidth([case, case2],
                       save=save_path)
